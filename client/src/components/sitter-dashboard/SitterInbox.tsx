...
export function SitterInbox() {
    const mockSitterMessages = [
        { id: 1, sender: "Abid Rahman", lastMessage: "Is Bruno allowed to sleep on the sofa?", time: "2m ago", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Abid", active: true },
        { id: 2, sender: "Sara Islam", lastMessage: "Thanks for the update on Mittens!", time: "1h ago", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Sara", active: false }
    ];

    return (
        <div className="flex h-[calc(100vh-65px)] bg-background animate-in fade-in duration-300">
            {/* Sidebar List */}
            <div className="w-80 border-r border-border h-full hidden lg:flex flex-col">
                ...
                <ScrollArea className="flex-1">
                    <div className="p-2 space-y-1">
                        {mockSitterMessages.map((msg) => (
                            <button
                                key={msg.id}
                                className={cn(
                                    "w-full p-3 rounded-md flex gap-3 transition-colors text-left",
                                    msg.active ? "bg-secondary" : "hover:bg-secondary/50"
                                )}
                            >
                                <Avatar className="w-9 h-9 border rounded-md shrink-0">
                                    <AvatarImage src={msg.avatar} />
                                    <AvatarFallback>{msg.sender[0]}</AvatarFallback>
                                </Avatar>
                                <div className="flex-1 overflow-hidden">
                                    <div className="flex justify-between items-baseline mb-0.5">
                                        <p className={cn("text-sm font-semibold truncate", msg.active ? "text-primary" : "text-foreground")}>{msg.sender}</p>
            ...
            <div className="flex-1 flex flex-col h-full bg-secondary/5">
                <div className="p-4 border-b border-border flex items-center gap-3 bg-background">
                    <Avatar className="w-8 h-8 border rounded-md">
                        <AvatarImage src={mockSitterMessages[0].avatar} />
                        <AvatarFallback>A</AvatarFallback>
                    </Avatar>
                    <div>
                        <p className="text-sm font-semibold leading-none">{mockSitterMessages[0].sender}</p>
                    ...
                    <div className="max-w-3xl mx-auto space-y-4">
                        <div className="flex justify-start">
                            <div className="bg-background border border-border p-3 rounded-lg rounded-tl-none shadow-sm max-w-sm text-sm font-medium leading-relaxed">
                                Hi! Is Bruno allowed to sleep on the sofa? He's very well behaved.
                            </div>
                        </div>
