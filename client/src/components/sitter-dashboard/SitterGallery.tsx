...
export function SitterGallery() {
    const images = [
        { id: 1, url: "https://images.unsplash.com/photo-1513360371669-4ada307f933b?auto=format&fit=crop&w=600", tag: "Main Atrium" },
        { id: 2, url: "https://images.unsplash.com/photo-1541599540903-21b128d75547?auto=format&fit=crop&w=600", tag: "Play Area" }
    ];
                ...
                {images.map((img) => (
                    <div key={img.id} className="aspect-4/5 rounded-xl overflow-hidden shadow-sm relative group border border-border bg-muted">
                        ...
                        <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                            <p className="text-white font-bold text-[10px] uppercase tracking-wider">{img.tag}</p>
                        </div>
                    </div>
                ))}

                <button
                    className="aspect-4/5 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center text-muted-foreground/40 gap-4 hover:border-primary/50 hover:bg-primary/5 transition-all group"
                >
                    <ImagePlus className="w-10 h-10 group-hover:scale-105 transition-transform" />
                    <span className="text-[10px] font-bold uppercase tracking-wider">New Perspective</span>
                </button>
            </div>
        </div>
    );
